...
class CronJob extends \yii\db\ActiveRecord
{
     ...
     * @param type $minutesDifference
     * @return array of datetimes
     */
    public static function initDatetimesInRange($start_datetime, $end_datetime, $minutesDifference = 1){
        $dates_in_range = [];
        $begin = new DateTime($start_datetime);
        $end = new DateTime($end_datetime);
        $endModify = $end->modify( '+'.$minutesDifference.' minute' );

        $interval = new DateInterval('PT1M'.$minutesDifference.'i');
        $daterange = new DatePeriod($begin, $interval ,$endModify);

        foreach($daterange as $date){
                $dates_in_range[$date->getTimestamp()] = $date->format("H:i");
        }
        return $dates_in_range;
    }
